...
const TasksTab = memo(({
  ...
  // Ensure we have fresh assignments when a highlight is requested (prevents missing highlight due to stale cache)
  useEffect(() => {
    if (highlightedAssignmentId) {
      fetchAssignments();
    }
  }, [highlightedAssignmentId, fetchAssignments]);
  ...
  const handleFileUpload = useCallback(async () => {
    ...
    const requiredCount = currentAssignment.required_files_count || 0;
    if (requiredCount > 0) {
      const currentlySubmitted = (currentAssignment.submitted_files || []).length;
      const newlyUploaded = uploadedFiles.length;
      if (currentlySubmitted + newlyUploaded !== requiredCount) {
        showError(`This task requires exactly ${requiredCount} file(s). You currently have ${currentlySubmitted} submitted and are trying to upload ${newlyUploaded}. Total must be exactly ${requiredCount}.`);
        return;
      }
    }
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
                  ...
                  {assignment.required_files_count > 0 && (
                    <span style={{ backgroundColor: 'var(--status-review)', color: 'var(--status-review-text)', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: '600', border: '1px solid var(--status-review-text)', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                      Requires {assignment.required_files_count} file(s)
                    </span>
                  )}
                </div>
